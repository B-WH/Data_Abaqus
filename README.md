...
1. 选择缓存 NPZ；程序自动读取同名配套 metadata，并列出其中的节点场和节点集。
2. 选择目标点 CSV/XLSX，勾选要查询的场输出，并按需选择缓存中的节点集。
3. 点击“开始查询”。该模式不需要 ODB、Abaqus、Step 或频率过滤，也不会在缓存无效时静默重读 ODB。

缓存模式下多个节点集取并集，未选择节点集时使用缓存中的全部节点。新生成的 schema v3 节点缓存会把装配级节点集成员压缩保存到 NPZ；旧缓存仍可查询全部节点，但不能重新选择节点集。默认查询输出位于缓存目录，名称为 `<目标点文件名>_point_data.npz`，且不能覆盖源缓存。
