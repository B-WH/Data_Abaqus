...
- `run_extract_odb.py` 是普通 Python 入口，负责 GUI、参数校验、调用 Abaqus 命令和后处理串联。
- `Extract_data_ODB.py` 必须由 Abaqus Python 执行，负责实际读取 `.odb`。
- `interpolate_odb_points.py` 是普通 Python 后处理脚本，读取已导出的 NPZ、metadata JSON 和目标点 CSV。

普通 Python 不能直接导入 Abaqus 的 `odbAccess`。如果直接运行 `Extract_data_ODB.py` 时提示缺少 `odbAccess`，应改用 Abaqus Python，例如：
...
```powershell
python run_extract_odb.py --odb data\test1.odb --fields U V A
```

带目标点导出：

```powershell
python run_extract_odb.py --odb data\test1.odb --points points.csv --point-fields U V
```
